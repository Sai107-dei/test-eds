...
function getUserInfo() {
  ...
  return fetch(currentUserEndpoint).then(response => response.json());
}

function getAuthorizables(userName) {
  var authorizablesEndpoint = `/bin/security/authorizables.json?filter=${encodeURIComponent(userName)}&_charset_=utf-8`;
  return fetch(authorizablesEndpoint).then(response => response.json());
}
...
(function() {
  getUserInfo()
    .then(data => {
      ...
      return getAuthorizables(userName);
    })
